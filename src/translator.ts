...
/**
 * Khởi tạo các regex mẫu để xóa nội dung trong các thẻ đặc biệt.
 * @returns Chuỗi chứa các regex mẫu, mỗi regex trên một dòng.
 */
function initializeDefaultRegexes(): string {
    // Danh sách các regex mẫu để xóa nội dung trong các thẻ đặc biệt
    const defaultRegexes = [
        // Xóa nội dung trong thẻ <plan>...</plan> nhưng giữ lại thẻ
        '(<plan>)[\\s\\S]*?(</plan>)',
        // Xóa nội dung trong thẻ <safety_check>...</safety_check> nhưng giữ lại thẻ
        '(<safety_check>)[\\s\\S]*?(</safety_check>)',
        // Các thẻ khác có thể xuất hiện trong web game
        '(<plot>)[\\s\\S]*?(</plot>)',
        '(<character>)[\\s\\S]*?(</character>)',
        '(<setting>)[\\s\\S]*?(</setting>)',
        '(<dialogue>)[\\s\\S]*?(</dialogue>)',
        // Thêm các regex khác cho các thẻ đặc biệt khác nếu cần
    ];

    return defaultRegexes.join('\n');
}
    ...
    for (const line of lines) {
        ...
        if (trimmedLine) {
            try {
                ...
                // Đảm bảo regex hoạt động với nội dung trên nhiều dòng
...
export function initializeTranslator() {
    ...
    if (regexTextarea) {
        // Kiểm tra xem có regex nào được lưu trong localStorage không
        const savedRegexes = localStorage.getItem('deletion-regexes');
        
        // Nếu không có regex nào được lưu, sử dụng các regex mẫu
        if (!savedRegexes || savedRegexes.trim() === '') {
            regexTextarea.value = initializeDefaultRegexes();
            // Lưu các regex mẫu vào localStorage
            localStorage.setItem('deletion-regexes', regexTextarea.value);
        } else {
            regexTextarea.value = savedRegexes;
        }
...
function translateBatch(originalTexts: string[]): Promise<string[]> {
    ...
    if (deletionRegexes.length > 0) {
        originalTexts = originalTexts.map(text => {
            ...
            for (const regex of deletionRegexes) {
                // Kiểm tra xem regex có chứa nhóm bắt (capturing groups) không
                const regexStr = regex.toString();
                if (regexStr.includes('(') && regexStr.includes(')')) {
                    // Nếu có nhóm bắt, giữ lại thẻ mở và thẻ đóng, chỉ xóa nội dung giữa chúng
                    processedText = processedText.replace(regex, '$1$2');
                } else {
                    // Nếu không có nhóm bắt, xóa toàn bộ kết quả khớp
...
async function translateNode(node: Node): Promise<void> {
    // Hybrid Approach:
    // Step 1: Clean the DOM directly. This catches content missed by the XHR interceptor,
    // such as content generated by client-side scripts without network requests.
    if (node.nodeType === 1 && deletionRegexes.length > 0) {
        const element = node as Element;
        ...
        for (const regex of deletionRegexes) {
            ...
            if (regexStr.includes('(') && regexStr.includes(')')) {
                // Nếu có nhóm bắt, giữ lại thẻ mở và thẻ đóng, chỉ xóa nội dung giữa chúng
                // Ví dụ: '(<plan>)[\s\S]*?(</plan>)' sẽ được thay thế bằng '$1$2'
                cleanedHTML = cleanedHTML.replace(regex, '$1$2');
            } else {
                // Nếu không có nhóm bắt, xóa toàn bộ kết quả khớp
    ...
    // Step 2: Proceed with translation on the (potentially) cleaned DOM.
