...
async function translateNode(node: Node): Promise<void> {
    // Step 1: Apply deletion regexes directly and safely to text nodes.
    // This is more robust than using innerHTML and avoids breaking event listeners.
    if (deletionRegexes.length > 0 && (node.nodeType === 1 || node.nodeType === 3)) {
    ...
    // Step 2: Proceed with translation on the (now cleaned) DOM.
